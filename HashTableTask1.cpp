...
const size_t HASH_TABLE_SIZE = 3571;
const size_t LIST_CAPACITY = 8;
...
bool ListSearch     (List* list, Pair* pair);

struct HashTable {
  List* list;
  size_t size;
  size_t (*HashFunction)(Pair*);
};
...
size_t HashFunction_1(Pair* pair);
size_t HashFunction_2(Pair* pair);
size_t HashFunction_3(Pair* pair);
size_t HashFunction_4(Pair* pair);
size_t HashFunction_5(Pair* pair);
size_t HashFunction_6(Pair* pair);
size_t HashFunction_7(Pair* pair);
...
void PrintfSizeList(HashTable* table, size_t number);

void TestHashFunction(size_t (*Hash)(Pair*), size_t number) {
	BufInfo* buf_info = (BufInfo*)calloc(1, sizeof(BufInfo));
	ReadFile(buf_info, "dict.txt");

	HashTable* table = (HashTable*)calloc(1, sizeof(HashTable));

	HashTableConstructor(table, Hash);
	FillHashTable(buf_info, table);
	PrintfSizeList(table, number);
...
int main() {
	TestHashFunction(HashFunction_1, 1);
	TestHashFunction(HashFunction_2, 2);
	TestHashFunction(HashFunction_3, 3);
	TestHashFunction(HashFunction_4, 4);
	TestHashFunction(HashFunction_5, 5);
	TestHashFunction(HashFunction_6, 6);
	TestHashFunction(HashFunction_7, 7);
  return 0;
}
...
size_t HashFunction_5(Pair* pair) {
  ...
  for (size_t i = 0; i < strlen(pair->key); ++i) {
    result = ((result << 1) + ((result >> 63) & 1)) ^ pair->key[i];
  }
...
size_t HashFunction_6(Pair* pair) {
  ...
  for (size_t i = 0; i < strlen(pair->key); ++i) {
    result = ((result >> 1) + ((result & 1) << 63)) ^ pair->key[i];
  }
...
void PrintfSizeList(HashTable* table, size_t number) {
    ...
    char buffer[128] = {};
    sprintf(buffer, "result%lu.txt", number);
