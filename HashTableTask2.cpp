...
void FindTranslate(BufInfo* translate, HashTable* table);
...
int main() {
  ...
  ReadFile(translate, "translate.txt");
  FindTranslate(translate, table);
...
void FindTranslate(BufInfo* translate, HashTable* table) {
  ...
  FILE* file = fopen("translate.html", "w");
  ...
  for (size_t i = 0; i < translate->buf_size;) {
    ...
    pair.key = temp_buf;
    pair.value = nullptr;
    if (HashTableSearch(table, &pair)) {
      fprintf(file, "<text title=\"%s\">%s</text>", pair.value, pair.key);
    } else {
      fprintf(file, "<text title=\"%s\">%s</text>", "not find", pair.key);
    }

    while (isalpha(translate->buf[i]) == 0) {
      if (translate->buf[i] != '\n') {
        fprintf(file, "<text>%c</text>", translate->buf[i]);
      } else {
        fprintf(file, "</p><p>");
      }
      i++;
